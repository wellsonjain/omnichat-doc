...
{% hint style="danger" %}
請特別注意！範例檔案中 「價格(Price)」 欄位：

1. 幣別應該要在金額數字後面，例如：9.99 USD 或 7.99 EUR 或 120 RM
2. 數字內 <mark style="color:red;">不支援 「逗號」</mark>，例如：1,350 TWD ❌ ⇒  1350 TWD ✅\
   因為後端是用逗號去做欄位分隔，所以在每個欄位裡面的資料都不建議使用逗號，否則影響檔案解析
{% endhint %}
