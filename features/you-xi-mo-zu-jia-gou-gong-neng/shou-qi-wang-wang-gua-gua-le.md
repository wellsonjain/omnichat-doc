...
按下右上角的『新增互動遊戲』，點選『手氣旺旺刮刮樂』後進到遊戲設定
...
3. 背景圖片：寬度為 1024 pixel，高度則會依照遊戲規則長度需求調整，建議將重要視覺元素往上放置，並進行測試查看實際顯示狀況。\
   ＊補充說明：\
   背景圖呈現完「遊戲規則」後，下方會預留約 **60px 的空白區域**，超過這個範圍的部分會被系統裁切，另外實際裁切位置也會因 **使用者裝置尺寸不同** 而有所變化。\
   由於每個品牌的遊戲規則文字字數不一，因此背景圖高度的部分，還請在設計時請特別注意，**背景圖底部若有重要資訊或視覺元素，建議往上移動，避免因裁切而無法完整顯示**。
...
{% hint style="warning" %}
1. 在『草稿階段』可進行測試，但測試紀錄會在正式發佈後被清除，所以當準備正式開始發送遊戲時，請按下按鈕『發布遊戲』。
