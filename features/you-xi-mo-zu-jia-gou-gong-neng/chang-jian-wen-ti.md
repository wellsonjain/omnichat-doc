...
#### Q：為什麼已經將 LIFF ID 放進『串接社群渠道』的欄位，且已按下『修改』，設定完遊戲後點擊開遊戲連結無法遊玩？
...
{% hint style="info" %}
* 若客人用電腦版開啟 LINE 遊戲連結，會顯示 QR Code 讓客人用手機掃描（會依照瀏覽器預設語系顯示文字）
* 若客人用手機瀏覽器開啟 LINE 遊戲連結，系統會強制客人進入中繼轉換頁面，讓客人開啟 LINE
{% endhint %}
...
#### Q：如果我是透過 LINE 市集來測試互動遊戲，中間要如何取消擴充功能（連動）呢？

A：您可以到 Omnichat 後台 「社群渠道」中的『串接社群渠道』，按下『解除串接』按鈕。即可將 LINE 官方帳號與 Omnichat 抽獎遊戲解除串接

{% hint style="danger" %}
如您取消串接則無法手動復原，如要重新串接互動遊戲，則請洽 Omnichat 客服人員。
{% endhint %}
...
A：承接上一提情境，因系統邏輯是，即使獎項數量被抽完，仍會按照後台設定而有機率抽中，此時抽中就會顯示未中獎的樣式以及未中獎訊息（如果有設定及開啟顯示的話）
