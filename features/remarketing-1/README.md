---
description: 官網顧客行銷包含三個功能：顧客分眾標籤/站內行銷訊息/通訊渠道再行銷推播，可以對網站的訪客進行行銷行為，以及能作為旅程的觸發條件。
---
...
1. **行銷活動名稱**
2. **狀態：**&#x6392;程中／進行中／停用中／已結束
3. **活動期間：**&#x82E5;有排程特定時間發布，則會顯示排程的時間區間
4. **訪問頁面：**&#x4EFB;意頁面/特定頁面
5. **總訪問次數：**&#x986F;示設定的訪問次數條件
6. **建立時間：**&#x986F;示行銷活動的建立時間
7. **動作：**&#x53EF;以執行行銷活動的編輯、複製、刪除，若要停用需要進入編輯頁面
8. **搜尋：**&#x53EF;以依照活動名稱搜尋
9. **更多功能：**&#x53EF;以進入官網顧客行銷的 「UTM 設定」、「統計圖表」，關於統計圖表的說明請參考[這裡](tong-ji-biao.md)
...
11. 點擊後可以新增一則行銷活動，共有 1 種自訂範本、5 種預設範本可以新增。預設範本會設定好相關的觸發條件、觸發內容，客製化後就可以發布使用

<figure><img src="../../.gitbook/assets/截圖 2025-10-29 上午9.44.21.png" alt=""><figcaption></figcaption></figure>
...
1. **行銷活動名稱：**&#x53EF;以輸入行銷活動名稱
2. **行銷活動期間：**
   1. **不限時間：**&#x767C;布活動後會運作直到手動停用為止
   2. **特定時間：**&#x8A2D;定時間區間內觸發活動
3. **訪問頁面：**
   1. **任意頁面：**&#x6709;安裝插件的網站全站都會顯示
   2. **特定頁面：**&#x8A2A;客符合造訪且有安裝插件的頁面才會觸發
...
5. **停留多久觸發動作：**&#x8A2D;定訪客符合訪問頁面及造訪次數後，停留多久時間觸發

{% hint style="success" %}
系統判斷是否符合造訪條件的優先順序為：

先判斷訪問頁面 >> 再判斷造訪次數 >> 最後是停留時間，舉例：
...
6. **排除對象：若訪客符合排除對象中設定的任一個頁面，則此行銷活動就不會觸發。**&#x4F8B;如：若您的行銷活動再行銷訊息是屬於發放優惠碼的訊息，這時可能會希望排除掉之前已經到過結帳完成頁面的人（表示此訪客曾經結帳），不希望已經結帳的訪客再得到優惠碼，則可以使用該條件
7. **觸發動作設定：**&#x8A2D;定當訪客符合左側設定的規則後，要觸發的內容。各個內容的說明請參考[這裡](chu-fa-dong-zuo-she-ding.md)
8. **儲存：**&#x7DE8;輯完成後，點擊儲存會成為 「草稿」，此時還不會顯示在官網中。若行銷活動發布後，儲存的按鈕會變成 「停用」 按鈕
9. **發布：**&#x9EDE;擊發布後，就會依照設定規則觸發內容給訪客
